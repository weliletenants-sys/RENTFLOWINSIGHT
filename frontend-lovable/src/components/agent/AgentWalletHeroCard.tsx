        ...
        <div className="relative z-10 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="portfolio-stat-cell-v2 rounded-xl px-2 py-3 text-center">
              <div className="flex items-center justify-center mb-1.5">
                <Users className="h-3.5 w-3.5 text-primary-foreground/60" />
              </div>
              <p className="text-xl font-black leading-none text-primary-foreground">{tenantsCount}</p>
              <p className="text-[8px] uppercase tracking-[0.14em] font-bold text-primary-foreground/50 mt-1.5">Tenants</p>
            </div>
            ...
            <div className="portfolio-stat-cell-v2 rounded-xl px-1.5 py-3 text-center overflow-hidden">
              ...
              <p className="text-sm font-black leading-none text-primary-foreground truncate" title={formatAmount(totalEarnings)}>
                {formatAmountCompact(totalEarnings)}
              </p>
              <p className="text-[8px] uppercase tracking-[0.14em] font-bold text-primary-foreground/50 mt-1.5">Earned</p>
            </div>

            {/* Commission — Opens sheet */}
            <button
              onClick={() => { hapticTap(); setShowWallet(true); }}
              className="portfolio-stat-cell-v2 rounded-xl px-1.5 py-3 text-center overflow-hidden hover:bg-primary-foreground/20 active:scale-95 transition-all cursor-pointer"
            >
              <div className="flex items-center justify-center mb-1.5">
                <Wallet className="h-3.5 w-3.5 text-emerald-400/80" />
              </div>
              <p className="text-sm font-black leading-none text-primary-foreground truncate" title={formatAmount(commissionBalance)}>
          ...
          <div className="flex items-center justify-between px-1 pt-1">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[9px] text-primary-foreground/40 font-medium">Welile Agent</span>
            </div>
            {territory && (
              <span className="text-[9px] text-primary-foreground/40 font-medium">{territory}</span>
            )}
          </div>
        </div>
      </div>
