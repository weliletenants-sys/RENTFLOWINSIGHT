...
BEGIN
  ...
  SELECT COALESCE(
    (SELECT SUM(CASE WHEN direction = 'cash_in' THEN amount ELSE -amount END)
     FROM general_ledger
     WHERE user_id = p_agent_id AND ledger_scope = 'wallet'
       AND category IN ('agent_float_funding', 'agent_float_topup', 'float_assignment', 'agent_float_refund')
    ), 0
  ) INTO v_float_balance;
  ...
  -- Check outstanding on rent request
  ...
  VALUES (p_agent_id, p_amount, 'cash_out', 'agent_float_funding', 'agent_collections', p_rent_request_id::TEXT,
  ...
  -- 4. Update rent request amount_repaid
  UPDATE rent_requests
  SET amount_repaid = amount_repaid + p_amount,
      status = CASE
        WHEN (amount_repaid + p_amount) >= total_repayment THEN 'completed'
        WHEN status = 'disbursed' THEN 'repaying'
        ELSE status
      END
  WHERE id = p_rent_request_id;

  -- 5. Record in agent_collections
  ...
  -- 6. Credit commission (10% via existing engine)
