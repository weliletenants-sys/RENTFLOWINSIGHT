-- Update the function to add notification on rejection
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    IF NEW.rejection_reason IS NOT NULL AND OLD.rejection_reason IS NULL AND OLD.loan_contribution IS NOT NULL AND OLD.loan_contribution > 0 THEN
      ...
      -- Send notification about rejection and limit reduction
      INSERT INTO public.notifications (user_id, title, message, type, metadata)
      VALUES (
        NEW.user_id,
        '❌ Receipt Rejected',
        'Your receipt was rejected: ' || COALESCE(NEW.rejection_reason, 'No reason provided') || '. Your loan limit has been reduced by UGX ' || ROUND(OLD.loan_contribution) || '.',
        'warning',
        jsonb_build_object(
          'receipt_id', NEW.id,
          'rejection_reason', NEW.rejection_reason,
          'limit_reduction', OLD.loan_contribution
        )
      );
    ...
    IF NEW.verified = true AND OLD.verified = false THEN
      ...
      VALUES (
        NEW.user_id,
        '✅ Receipt Verified!',
        'Your receipt has been verified! Your loan limit of UGX ' || ROUND(COALESCE(OLD.loan_contribution, 0)) || ' is now confirmed.',
        'success',
        jsonb_build_object(
          'receipt_id', NEW.id,
          'loan_contribution', COALESCE(OLD.loan_contribution, 0)
        )
      );
    END IF;
