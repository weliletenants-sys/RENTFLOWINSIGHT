...
BEGIN
  ...
  -- ─── PAYMENT HISTORY (35%) — uses rent_amount + amount_repaid ───
  SELECT COUNT(*),
         COALESCE(SUM(COALESCE(amount_repaid, 0)), 0),
         COALESCE(SUM(CASE WHEN status NOT IN ('completed','repaid','cancelled','rejected')
                           THEN GREATEST(COALESCE(rent_amount,0) - COALESCE(amount_repaid,0), 0)
                           ELSE 0 END), 0)
  ...
  -- ─── WALLET ACTIVITY (25%) — uses sender_id/recipient_id ───
  BEGIN
    SELECT COALESCE(balance, 0) INTO v_wallet_balance FROM public.wallets WHERE user_id = v_user_id LIMIT 1;
  EXCEPTION WHEN undefined_table OR undefined_column THEN v_wallet_balance := 0; END;

  BEGIN
    SELECT
      COALESCE(SUM(CASE WHEN recipient_id = v_user_id THEN amount ELSE 0 END), 0),
      COALESCE(SUM(CASE WHEN sender_id = v_user_id THEN amount ELSE 0 END), 0),
      COUNT(*)
    INTO v_received_180d, v_sent_180d, v_txn_count
    FROM public.wallet_transactions
    WHERE (recipient_id = v_user_id OR sender_id = v_user_id)
      AND created_at > now() - interval '180 days';
  EXCEPTION WHEN undefined_table OR undefined_column THEN v_txn_count := 0; END;
  ...
  v_portfolio_score := CASE
    ...
    WHEN v_portfolio_value >= 20000000  THEN 40
    WHEN v_portfolio_value >= 10000000  THEN 35
    WHEN v_portfolio_value >= 5000000   THEN 30
    WHEN v_portfolio_value >= 2000000   THEN 25
    WHEN v_portfolio_value >= 1000000   THEN 20
    WHEN v_portfolio_value >= 500000    THEN 15
    WHEN v_portfolio_value >= 200000    THEN 12
    WHEN v_portfolio_value >= 100000    THEN 10
