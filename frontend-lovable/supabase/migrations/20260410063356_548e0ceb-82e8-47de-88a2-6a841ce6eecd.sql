...
BEGIN
  ...
  LEFT JOIN (
    SELECT assigned_agent_id, COUNT(DISTINCT user_id) AS tenants_count
    FROM rent_requests
  ...
  WHERE ur.role = 'agent'
    AND (search_query = '' OR p.full_name ILIKE '%' || search_query || '%')
  ORDER BY
    CASE WHEN sort_field = 'full_name' AND sort_dir = 'asc' THEN p.full_name END ASC,
    CASE WHEN sort_field = 'full_name' AND sort_dir = 'desc' THEN p.full_name END DESC,
