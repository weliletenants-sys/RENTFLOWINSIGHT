...
BEGIN
  ...
  VALUES (
    ...
    v_daily_txn, COALESCE(v_roles, '{}'), now()
  )
  ON CONFLICT (stat_date) DO UPDATE SET
    total_users = EXCLUDED.total_users, active_users_30d = EXCLUDED.active_users_30d,
    new_users_today = EXCLUDED.new_users_today, retention_pct = EXCLUDED.retention_pct,
    referral_pct = EXCLUDED.referral_pct, daily_transaction_volume = EXCLUDED.daily_transaction_volume,
    users_by_role = EXCLUDED.users_by_role, updated_at = now();
END;
$$;
