...
-- 1. Tighten trigger idempotency guard with explicit category filter
...
BEGIN
  ...
  -- DETERMINISTIC IDEMPOTENCY: exact match on source + category + scope
  IF EXISTS (
...
-- 2. Add idempotency_key to create_ledger_transaction RPC
...
BEGIN
  -- IDEMPOTENCY CHECK: if key provided and already used, return existing group
  ...
  LOOP
    ...
    -- When idempotency_key provided, stamp it as reference_id on ALL entries
