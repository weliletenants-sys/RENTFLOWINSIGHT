-- Fix the notify_onboarding_milestone function where current_month is TEXT but target_month column is DATE
...
BEGIN
  -- Get current month as DATE (first day of current month)
  current_month_date := date_trunc('month', now())::DATE;
  
  -- Get the monthly target (use DATE type for comparison)
  SELECT target_count INTO v_target_count
  FROM public.onboarding_targets
  WHERE target_month = current_month_date;
  ...
  progress_percentage := (current_count * 100) / v_target_count;
  previous_percentage := (previous_count * 100) / v_target_count;
  ...
  IF milestone_hit IS NOT NULL THEN
    ...
    LOOP
      ...
      VALUES (
        ...
        END,
        CASE milestone_hit
          WHEN 100 THEN 'Amazing! The team has reached 100% of this month''s onboarding target (' || current_count || '/' || v_target_count || ' users)!'
          WHEN 75 THEN 'Great progress! The team has reached 75% of this month''s onboarding target (' || current_count || '/' || v_target_count || ' users).'
          WHEN 50 THEN 'Good momentum! The team is halfway to this month''s onboarding target (' || current_count || '/' || v_target_count || ' users).'
        END,
        CASE milestone_hit
