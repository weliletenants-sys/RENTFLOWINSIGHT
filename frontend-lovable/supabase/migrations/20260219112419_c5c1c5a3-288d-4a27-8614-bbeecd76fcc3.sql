...
-- Extend the existing trigger function to also boost the AGENT's credit limit
-- when they fund a tenant's rent request.
...
BEGIN
  ...
  THEN
    ...
    -- Upsert tenant credit_access_limits
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'bonus_added',      v_tenant_bonus,
        'new_total_limit',  v_new_total,
    ...
    -- ─── AGENT BOOST ────────────────────────────────────────────────────────
    -- 5% of rent, min 5k, max 150k — reward for active facilitation
    ...
    VALUES (
      NEW.agent_id,
      '🚀 Your Loan Limit Grew!',
      'You facilitated a UGX ' || TO_CHAR(NEW.rent_amount, 'FM999,999,999')
        || ' rent payment — your available loan limit has increased by UGX '
        || TO_CHAR(v_agent_bonus, 'FM999,999,999')
        || '. Keep facilitating rent payments to unlock up to UGX 30M!',
      'success',
      jsonb_build_object(
        'type',             'credit_limit_increase',
        'bonus_added',      v_agent_bonus,
        'new_total_limit',  v_new_total,
        'rent_request_id',  NEW.id,
        'rent_amount',      NEW.rent_amount
      )
    ...
    RAISE LOG '[boost_tenant_credit] Tenant % boosted by %, Agent % boosted by % (rent=%,  request=%)',
      NEW.tenant_id, v_tenant_bonus, NEW.agent_id, v_agent_bonus, NEW.rent_amount, NEW.id;
  END IF;
...
-- Ensure trigger is attached (recreate to pick up function changes)
