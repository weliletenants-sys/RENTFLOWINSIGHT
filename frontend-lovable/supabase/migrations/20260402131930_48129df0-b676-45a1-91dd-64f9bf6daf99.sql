...
-- Fix credit_agent_rent_commission: correct column names + platform cash_out double-entry
...
BEGIN
  ...
  IF v_source_amount > 0 THEN
    IF NOT EXISTS (
      ...
      -- Platform cash_out (double-entry debit from platform)
      ...
      VALUES (v_source_agent_id, v_source_amount, 'cash_out', 'agent_commission', 'commission_engine', p_rent_request_id,
        'Platform → Source agent 2% commission', 'platform', v_txn_group);

      -- Agent wallet cash_in (triggers sync_wallet_from_ledger)
      INSERT INTO general_ledger (user_id, amount, direction, category, source_table, source_id, description, ledger_scope, transaction_group_id)
  ...
  IF v_manager_amount > 0 THEN
    IF NOT EXISTS (
      ...
      -- Platform cash_out
      INSERT INTO general_ledger (user_id, amount, direction, category, source_table, source_id, description, ledger_scope, transaction_group_id)
      VALUES (v_manager_agent_id, v_manager_amount, 'cash_out', 'agent_commission', 'commission_engine', p_rent_request_id,
        'Platform → Manager commission', 'platform', v_txn_group);

      -- Agent wallet cash_in
      INSERT INTO general_ledger (user_id, amount, direction, category, source_table, source_id, description, ledger_scope, transaction_group_id)
  ...
  IF v_recruiter_amount > 0 AND v_recruiter_id IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      -- Platform cash_out
      INSERT INTO general_ledger (user_id, amount, direction, category, source_table, source_id, description, ledger_scope, transaction_group_id)
      VALUES (v_recruiter_id, v_recruiter_amount, 'cash_out', 'agent_commission', 'commission_engine', p_rent_request_id,
        'Platform → Recruiter override 2%', 'platform', v_txn_group);
...
-- Fix credit_agent_event_bonus: correct column names + platform cash_out double-entry
...
BEGIN
  ...
  -- Platform cash_out (double-entry debit)
  INSERT INTO general_ledger (user_id, amount, direction, category, source_table, source_id, description, ledger_scope, transaction_group_id)
  VALUES (p_agent_id, v_amount, 'cash_out', 'agent_commission', 'commission_engine', COALESCE(p_source_id::UUID, gen_random_uuid()),
    'Platform → ' || v_description, 'platform', v_txn_group);
