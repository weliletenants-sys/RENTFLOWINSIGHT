...
BEGIN
  ...
  with_collections AS (
    SELECT
      ta.uid,
      ta.*,
      COALESCE(cs.collection_count, 0) AS paid,
      CASE WHEN ta.total_rent > 0 AND ta.total_repaid < ta.total_rent
        THEN GREATEST(0, FLOOR((ta.total_rent - ta.total_repaid) / GREATEST(ta.total_rent / GREATEST(ta.total_requests, 1) / 30, 1)) - COALESCE(cs.collection_count, 0))
        ELSE 0 END::BIGINT AS missed,
      CASE WHEN COALESCE(cs.collection_count, 0) > 0
        THEN LEAST(100, ROUND(
          (ta.total_repaid / GREATEST(ta.total_rent, 1)) * 70 +
          (ta.fully_repaid_count::NUMERIC / GREATEST(ta.total_requests, 1)) * 30
        , 1))
        ELSE 50 END AS health
    FROM tenant_agg ta
    LEFT JOIN (
      SELECT ac.tenant_id AS uid, COUNT(*) AS collection_count
...
BEGIN
  ...
  combined AS (
    SELECT
      ...
      -- missed = estimated expected payments minus actual collections
      GREATEST(0,
        CASE WHEN COALESCE(tr.total_rent_amount, 0) > 0
          THEN FLOOR((tr.total_rent_amount - tr.total_repaid) / GREATEST(tr.total_rent_amount / GREATEST(tr.total_requests, 1) / 30, 1))
          ELSE 0 END - COALESCE(cs.on_time_payments, 0)
      )::BIGINT AS missed_payments,
      COALESCE(cs.on_time_payments, 0) AS on_time_payments,
      CASE WHEN COALESCE(tr.total_rent_amount, 0) > 0
        THEN LEAST(100, ROUND(
          (COALESCE(tr.total_repaid, 0) / GREATEST(tr.total_rent_amount, 1)) * 70 +
    ...
    LEFT JOIN collection_stats cs ON cs.uid = p.id
