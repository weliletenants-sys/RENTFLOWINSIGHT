...
BEGIN
  ...
  -- Referrals get the lion's share: 30 sign-ups via shared link = full 18 pts
  v_referrals_score := LEAST(18, v_referrals_count * 0.6);

  -- "Top Referrer" badge once an agent crosses 25 referrals
  v_top_referrer := v_referrals_count >= 25;

  v_network_score := LEAST(25,
      v_referrals_score
    + LEAST(3.0, v_subagents_all * 0.3 + v_sub_agents * 0.2)
    + LEAST(2.0, v_tenants_onboarded * 0.25)
    + LEAST(1.0, v_partners_managed * 0.5)
    + LEAST(0.7, v_landlords_registered * 0.4)
    + LEAST(0.3, v_promissory_count * 0.25)
  );
  ...
  -- Portfolio tier (rebalanced for 30-pt cap; max tier ~19)
  ...
  RETURN jsonb_build_object(
    ...
    'trust', jsonb_build_object(
      ...
      'weights', jsonb_build_object(
        'supporter', 30, 'payment', 15, 'wallet', 10, 'network', 25,
