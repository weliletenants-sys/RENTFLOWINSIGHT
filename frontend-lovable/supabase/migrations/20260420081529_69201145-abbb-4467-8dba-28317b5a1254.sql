-- =====================================================================
-- Agent network trust signals: count every managed relationship
-- =====================================================================
...
DECLARE
  ...
  v_sub_agents int := 0;              -- legacy: verified-only (kept for bonus)
  v_subagents_all int := 0;           -- NEW: all statuses
  v_tenants_onboarded int := 0;
  v_partners_managed int := 0;        -- NEW
  v_landlords_registered int := 0;    -- NEW
  v_promissory_count int := 0;        -- NEW
  v_network_signals_total int := 0;   -- aggregate for data_points
...
BEGIN
  ...
  -- ============ NETWORK (10) — every managed relationship counts ============
  ...
  v_network_score := LEAST(10,
    ...
    + LEAST(2.0, v_subagents_all          * 0.3 + v_sub_agents * 0.2)
    + LEAST(2.0, v_tenants_onboarded      * 0.25)
    + LEAST(1.5, v_partners_managed       * 0.5)
  ...
  IF v_portfolio_value > 0 THEN
    v_supporter_score := v_supporter_score + (
      CASE
        WHEN v_portfolio_value >= 100000000 THEN 25
        WHEN v_portfolio_value >= 50000000  THEN 23
        WHEN v_portfolio_value >= 20000000  THEN 21
        WHEN v_portfolio_value >= 10000000  THEN 19
        WHEN v_portfolio_value >= 5000000   THEN 17
        WHEN v_portfolio_value >= 2000000   THEN 15
        WHEN v_portfolio_value >= 1000000   THEN 12
        WHEN v_portfolio_value >= 500000    THEN 9
        WHEN v_portfolio_value >= 200000    THEN 6
        WHEN v_portfolio_value >= 50000     THEN 3
        ELSE 1
      END
    );
...
-- =====================================================================
-- Lightweight RPC for Agent Ops "Your network builds your trust" panel
-- =====================================================================
CREATE OR REPLACE FUNCTION public.get_agent_network_summary(p_agent_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
DECLARE
  ...
  v_subagents_verified int := 0;
  v_tenants int := 0;
  v_partners int := 0;
  v_landlords int := 0;
  v_promissory int := 0;
  v_pts_referrals numeric := 0;
  v_pts_subagents numeric := 0;
  v_pts_tenants numeric := 0;
  v_pts_partners numeric := 0;
  v_pts_landlords numeric := 0;
  v_pts_promissory numeric := 0;
  v_total_pts numeric := 0;
BEGIN
  -- Permission: self or staff
  IF auth.uid() IS NULL OR (auth.uid() <> p_agent_id AND NOT EXISTS (
    SELECT 1 FROM staff_permissions WHERE user_id = auth.uid() AND COALESCE(is_active, true)
  )) THEN
    RETURN jsonb_build_object('error', 'forbidden');
  END IF;

  SELECT COUNT(*) INTO v_referrals FROM profiles WHERE referrer_id = p_agent_id;

  BEGIN
    SELECT COUNT(*) INTO v_subagents_all FROM agent_subagents WHERE parent_agent_id = p_agent_id;
    SELECT COUNT(*) INTO v_subagents_verified FROM agent_subagents WHERE parent_agent_id = p_agent_id AND status = 'verified';
  EXCEPTION WHEN OTHERS THEN v_subagents_all := 0; v_subagents_verified := 0; END;

  BEGIN
    SELECT COUNT(DISTINCT tenant_id) INTO v_tenants
      FROM rent_requests WHERE agent_id = p_agent_id AND tenant_id IS NOT NULL;
  EXCEPTION WHEN OTHERS THEN v_tenants := 0; END;
  ...
  BEGIN
    SELECT COUNT(*) INTO v_partners
     ...
     WHERE agent_id = p_agent_id
       AND status IN ('active','matured','pending','pending_approval');
  EXCEPTION WHEN OTHERS THEN v_partners := 0; END;

  BEGIN
    SELECT COUNT(*) INTO v_landlords FROM landlords WHERE registered_by = p_agent_id;
  EXCEPTION WHEN OTHERS THEN v_landlords := 0; END;

  BEGIN
    SELECT COUNT(*) INTO v_promissory FROM promissory_notes WHERE agent_id = p_agent_id;
  EXCEPTION WHEN OTHERS THEN v_promissory := 0; END;

  v_pts_referrals  := LEAST(3.0, v_referrals * 0.4);
  v_pts_subagents  := LEAST(2.0, v_subagents_all * 0.3 + v_subagents_verified * 0.2);
  v_pts_tenants    := LEAST(2.0, v_tenants * 0.25);
  v_pts_partners   := LEAST(1.5, v_partners * 0.5);
  v_pts_landlords  := LEAST(1.0, v_landlords * 0.4);
  v_pts_promissory := LEAST(0.5, v_promissory * 0.25);
  v_total_pts := LEAST(10,
    v_pts_referrals + v_pts_subagents + v_pts_tenants
    + v_pts_partners + v_pts_landlords + v_pts_promissory);
  ...
  RETURN jsonb_build_object(
    ...
    'counts', jsonb_build_object(
      ...
      'sub_agents', v_subagents_all,
      'sub_agents_verified', v_subagents_verified,
      'tenants', v_tenants,
      'partners', v_partners,
      'landlords', v_landlords,
      'promissory_notes', v_promissory,
      'total', v_referrals + v_subagents_all + v_tenants + v_partners + v_landlords + v_promissory
    ),
    'points', jsonb_build_object(
      'referrals', ROUND(v_pts_referrals, 2),
      'sub_agents', ROUND(v_pts_subagents, 2),
      'tenants', ROUND(v_pts_tenants, 2),
      'partners', ROUND(v_pts_partners, 2),
      'landlords', ROUND(v_pts_landlords, 2),
      'promissory_notes', ROUND(v_pts_promissory, 2),
      'total', ROUND(v_total_pts, 2),
      'max', 10
    ),
    'generated_at', NOW()
  );
...
GRANT EXECUTE ON FUNCTION public.get_agent_network_summary(uuid) TO authenticated;

-- =====================================================================
-- Backfill cached scores so existing agents see the new formula immediately
-- =====================================================================
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_proc
    WHERE proname = 'recompute_trust_scores_batch' AND pronamespace = 'public'::regnamespace
  ) THEN
    PERFORM public.recompute_trust_scores_batch(50000);
  END IF;
END $$;