-- ============================================================
-- Agent Performance Booster
-- ============================================================

-- Helper: snapshot of an agent's tenant-payment quality
CREATE OR REPLACE FUNCTION public.compute_agent_performance(p_agent_id uuid)
RETURNS TABLE (
  qualifying_tenants int,
  healthy_tenants int,
  healthy_ratio numeric,
  collection_rate numeric,
  monthly_book numeric
)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
DECLARE
  v_qualifying int := 0;
  v_healthy int := 0;
  v_sum_expected numeric := 0;
  v_sum_collected numeric := 0;
BEGIN
  -- One grouped pass over rent_requests; uses idx_rent_requests_agent_id.
  -- "qualifying" = active disbursed tenancies older than 30 days
  -- "healthy"    = collected_30d (proxy: amount_repaid since disbursement, normalised to 30d) ≥ 50% expected_30d
  WITH active_tenants AS (
    SELECT
      ...
      rr.amount_repaid,
      rr.disbursed_at,
      rr.daily_repayment * 30 AS expected_30d,
      LEAST(
        rr.daily_repayment * 30,
        -- Pro-rate amount_repaid to a 30-day window (fall back to actual if tenancy <= 30d, but we filter that out below)
        (rr.amount_repaid / GREATEST(EXTRACT(EPOCH FROM (now() - rr.disbursed_at)) / 86400.0, 1.0)) * 30
      ) AS collected_30d
    FROM rent_requests rr
    WHERE rr.agent_id = p_agent_id
      AND rr.status IN ('disbursed','funded','approved')
      AND rr.tenancy_status = 'active'
      AND rr.disbursed_at IS NOT NULL
      AND rr.disbursed_at < now() - interval '30 days'
      AND rr.daily_repayment > 0
  )
  SELECT
    COUNT(*)::int,
    COUNT(*) FILTER (
      WHERE expected_30d > 0
        AND (collected_30d / expected_30d) >= 0.50
    )::int,
    COALESCE(SUM(expected_30d), 0),
    COALESCE(SUM(collected_30d), 0)
  INTO v_qualifying, v_healthy, v_sum_expected, v_sum_collected
  FROM active_tenants;

  qualifying_tenants := v_qualifying;
  healthy_tenants    := v_healthy;
  healthy_ratio      := CASE WHEN v_qualifying > 0 THEN ROUND(v_healthy::numeric / v_qualifying, 4) ELSE 0 END;
  collection_rate    := CASE WHEN v_sum_expected > 0 THEN LEAST(1.0, ROUND(v_sum_collected / v_sum_expected, 4)) ELSE 0 END;
  monthly_book       := COALESCE(v_sum_expected, 0);

  RETURN NEXT;
END;
$$;

GRANT EXECUTE ON FUNCTION public.compute_agent_performance(uuid) TO authenticated, anon, service_role;

-- ============================================================
-- Replace get_user_trust_profile to include Agent Performance
-- ============================================================
...
DECLARE
  ...
  v_sub_agents int := 0;
  v_subagents_all int := 0;
  v_tenants_onboarded int := 0;
  v_partners_managed int := 0;
  v_landlords_registered int := 0;
  v_promissory_count int := 0;
  v_network_signals_total int := 0;
  v_top_referrer boolean := false;
  ...
  -- Agent performance booster
  v_ap_qualifying int := 0;
  v_ap_healthy int := 0;
  v_ap_ratio numeric := 0;
  v_ap_collection_rate numeric := 0;
  v_ap_monthly_book numeric := 0;
  v_agent_perf_score numeric := 0;
  v_agent_term numeric := 0;
  v_top_performing boolean := false;
...
BEGIN
  ...
  -- ============ NETWORK (15) — re-weighted from 25 to free 10 pts for Agent Performance ============
  SELECT COUNT(*) INTO v_referrals_count FROM profiles WHERE referrer_id = v_user_id;

  BEGIN
    SELECT COUNT(*) INTO v_subagents_all
      FROM agent_subagents WHERE parent_agent_id = v_user_id;
    SELECT COUNT(*) INTO v_sub_agents
      FROM agent_subagents WHERE parent_agent_id = v_user_id AND status = 'verified';
  EXCEPTION WHEN OTHERS THEN v_subagents_all := 0; v_sub_agents := 0; END;
  ...
  BEGIN
    SELECT COUNT(*) INTO v_partners_managed
      FROM investor_portfolios
     WHERE agent_id = v_user_id
       AND status IN ('active','matured','pending','pending_approval');
  EXCEPTION WHEN OTHERS THEN v_partners_managed := 0; END;

  BEGIN
    SELECT COUNT(*) INTO v_landlords_registered
      FROM landlords WHERE registered_by = v_user_id;
  EXCEPTION WHEN OTHERS THEN v_landlords_registered := 0; END;

  BEGIN
    SELECT COUNT(*) INTO v_promissory_count
      FROM promissory_notes WHERE agent_id = v_user_id;
  EXCEPTION WHEN OTHERS THEN v_promissory_count := 0; END;

  -- Referrals scaled down to keep them dominant inside the smaller 15-pt band (was 18 of 25 → now 11 of 15)
  v_referrals_score := LEAST(11, v_referrals_count * 0.4);

  v_top_referrer := v_referrals_count >= 25;

  v_network_score := LEAST(15,
      v_referrals_score
    + LEAST(2.0, v_subagents_all * 0.25 + v_sub_agents * 0.15)
    + LEAST(1.0, v_tenants_onboarded * 0.15)
    + LEAST(0.6, v_partners_managed * 0.3)
    + LEAST(0.3, v_landlords_registered * 0.25)
    + LEAST(0.1, v_promissory_count * 0.15)
  );

  v_network_signals_total :=
      v_referrals_count + v_subagents_all + v_tenants_onboarded
    + v_partners_managed + v_landlords_registered + v_promissory_count;

  IF v_network_signals_total > 0 THEN
    v_data_points := v_data_points + LEAST(4, GREATEST(1, v_network_signals_total / 5));
  END IF;

  -- ============ AGENT PERFORMANCE (10) — NEW ============
  SELECT qualifying_tenants, healthy_tenants, healthy_ratio, collection_rate, monthly_book
    INTO v_ap_qualifying, v_ap_healthy, v_ap_ratio, v_ap_collection_rate, v_ap_monthly_book
    FROM compute_agent_performance(v_user_id);

  IF v_ap_qualifying >= 3 THEN
    v_agent_perf_score :=
        LEAST(6, v_ap_ratio * 6)
      + LEAST(3, v_ap_collection_rate * 3)
      + LEAST(1, LOG(GREATEST(v_ap_qualifying, 1)) * 0.5);
    v_agent_perf_score := LEAST(10, v_agent_perf_score);
    v_data_points := v_data_points + LEAST(3, v_ap_qualifying / 5 + 1);

    -- Vouch limit boost: 40% of healthy monthly book
    v_agent_term := v_ap_monthly_book * v_ap_ratio * 0.4;
  END IF;

  v_top_performing := (v_ap_ratio >= 0.80 AND v_ap_qualifying >= 10);
  ...
  v_behavior_score :=
      LEAST(2.0, v_visits_worship * 0.25)
    + LEAST(1.25, v_visits_mall * 0.125)
    + LEAST(1.0, v_visits_restaurant * 0.1)
    + LEAST(0.75, v_visits_hotel * 0.125)
    + LEAST(1.25, v_wallet_shop_count * 0.15);

  v_behavior_score := LEAST(5, v_behavior_score);
  ...
  IF v_portfolio_value > 0 THEN
    v_supporter_score := v_supporter_score + (
      CASE
        WHEN v_portfolio_value >= 100000000 THEN 19
        WHEN v_portfolio_value >= 50000000  THEN 17
        WHEN v_portfolio_value >= 20000000  THEN 16
        WHEN v_portfolio_value >= 10000000  THEN 14
        WHEN v_portfolio_value >= 5000000   THEN 13
        WHEN v_portfolio_value >= 2000000   THEN 11
        WHEN v_portfolio_value >= 1000000   THEN 9
        WHEN v_portfolio_value >= 500000    THEN 7
        WHEN v_portfolio_value >= 200000    THEN 4
        WHEN v_portfolio_value >= 50000     THEN 2
        ELSE 1
      END
    );
  ...
  IF v_roi_paid_180d > 0 THEN
    v_supporter_score := v_supporter_score + LEAST(7, (LOG(GREATEST(v_roi_paid_180d, 10))::numeric * 1.0));
    v_data_points := v_data_points + 2;
  END IF;

  IF v_portfolio_active_count > 0 THEN
    v_supporter_score := v_supporter_score + LEAST(4, v_portfolio_active_count * 0.8);
  END IF;

  v_supporter_score := LEAST(30, v_supporter_score);
  ...
  v_trust_score := LEAST(100,
    ...
    + v_agent_perf_score + v_verification_score + v_behavior_score + v_landlord_score
  );
  ...
  RETURN jsonb_build_object(
    ...
    'trust', jsonb_build_object(
      ...
      'breakdown', jsonb_build_object(
        ...
        'referrals', ROUND(v_referrals_score, 1),
        'agent_performance', ROUND(v_agent_perf_score, 1),
        'verification', ROUND(v_verification_score, 1),
        'behavior', ROUND(v_behavior_score, 1),
        'landlord', ROUND(v_landlord_score, 1)
      ),
      'weights', jsonb_build_object(
        'supporter', 30, 'payment', 15, 'wallet', 10, 'network', 15,
        'agent_performance', 10,
        'verification', 10, 'behavior', 5, 'landlord', 5
      )
    ),
    ...
    'network', jsonb_build_object(
      ...
      'referrals_score', ROUND(v_referrals_score, 1),
      'referrals_max', 11,
      'top_referrer', v_top_referrer,
      'sub_agents', v_subagents_all,
      'sub_agents_verified', v_sub_agents,
      'tenants_onboarded', v_tenants_onboarded,
      'partners_managed', v_partners_managed,
      'landlords_registered', v_landlords_registered,
      'promissory_notes', v_promissory_count,
      'total_relationships', v_network_signals_total,
      'portfolio_value', v_portfolio_value
    ),
    'agent_performance', jsonb_build_object(
      'qualifying_tenants', v_ap_qualifying,
      'healthy_tenants', v_ap_healthy,
      'healthy_ratio', v_ap_ratio,
      'collection_rate', v_ap_collection_rate,
      'monthly_book', v_ap_monthly_book,
      'agent_term', v_agent_term,
      'top_performing', v_top_performing
    ),
...
-- ============================================================
-- Public RPC: expose agent_performance block (sanitised)
-- ============================================================
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'agent_performance', jsonb_build_object(
      'qualifying_tenants', v_full->'agent_performance'->'qualifying_tenants',
      'healthy_tenants', v_full->'agent_performance'->'healthy_tenants',
      'healthy_ratio', v_full->'agent_performance'->'healthy_ratio',
      'top_performing', v_full->'agent_performance'->'top_performing'
    ),
    'behavior', jsonb_build_object(
