...
export function COOPartnerWithdrawalApprovals() {
  const { user } = useAuth();
  ...
  const fetchRequests = useCallback(async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('status', 'partner_ops_approved')
      ...
      console.error('COO partner withdrawal fetch error:', e);
      toast.error('Failed to load partner withdrawal requests');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      await supabase.from('audit_logs').insert({
        user_id: user.id,
        action_type: 'partner_withdrawal_operations_clearance_rejected',
        record_id: selected.id,
        table_name: 'investment_withdrawal_requests',
  ...
  return (
    <>
        ...
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <CardTitle className="text-sm flex items-center gap-2">
              ...
              Operations Clearance — Partner Withdrawals
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <p className="text-[10px] text-muted-foreground">
                    Portfolio reviewed {req.partner_ops_approved_at ? formatDistanceToNow(new Date(req.partner_ops_approved_at), { addSuffix: true }) : ''}
                  </p>
      ...
      <AlertDialog open={approveOpen} onOpenChange={setApproveOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Grant Operations Clearance?</AlertDialogTitle>
            <AlertDialogDescription>
              Forward <strong>{selected ? formatCurrency(selected.amount) : ''}</strong> for {selected?.user?.full_name} to Treasury for payout.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleApprove} disabled={!!processing}>
              {processing ? 'Processing...' : 'Clear & Forward'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
